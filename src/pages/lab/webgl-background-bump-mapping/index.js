  ...
  <div>
    <StandardPage>
      <ReadableSection>
        ...
        <h1 className="page-title">WebGL Background Bump Mapping</h1>
        <p className="page-tagline">12 May, 2012</p>
        <h3>What is it?</h3>
        <p>Background textures have always been fairly flat and static things.  I set out to increase their (apparent) depth.</p>
        <p>It's not entirely practical, and in retrospect I'm not sure why I created it, but here it is!  I think it's kind of neat and kind of pretty.</p>
        <p>This project features WebGL, bump mapping, and the Phong reflection model to 'enhance' a websites background.</p>
        <h3>Overview</h3>
        ...
        <p>Subtle tiling textures are care of <a href='http://subtlepatterns.com/' target='_blank'>subtlepatterns.com</a></p>
        <h3>Graphics Model</h3>
        <p>The image is represented in the XY plane.  The normal is simply <code>(0,0,1)</code>.  This allows us to easily place the light somewhere in the <code>(0,0,+z)</code> area, and use the mouse to control the X and Y positions of the light.  The Phong reflection model is used for shading.  The specular component is messed up, as you'll see if you increase rho.  Since the viewport is defined to be between <code>(-1,-1)x(1,1)</code>, we can easily reason about the position of the light in this space.  <code>(-1,-1)</code> being the bottom right, <code>(1,1)</code> being the top right.  An offset can be applied to the mouse's modifier, allowing us to position the light off-screen but still have the mouse affect the shading; this can make the effect subtler.</p>
        <h3>Bump mapping</h3>
