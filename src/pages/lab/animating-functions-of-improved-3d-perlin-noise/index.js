...
const IndexPage = () => (
  <StandardPage twoColumn>
    <ReadableSection>
      ...
      <h1 className="page-title">Animating Functions of Improved 3D Perlin Noise</h1>
      <p className="page-tagline">27 Oct, 2011 &ndash; a sleepless night</p>
      <h3>Overview</h3>
      <p>In this experiment, I made a quick port of Ken Perlin's <a href="http://mrl.nyu.edu/~perlin/noise/" target="_blank">classical noise</a> in 3 dimensions. Due to its continuity properties, we can take a 2D cross-section and step through the 3rd dimension in time. The end result is a continuous animation.</p>
      ...
      <ul>
        ...
        <li>Chrome 15 seemed to achieve >30FPS, or if not, the visual stutter wasn't too noticeable.</li>
      </ul>
      <p>In the WebGL shader implementation, on a 1GHz machine:</p>
      <ul>
        <li>Firefox 7 easily seems to achieve >30FPS on a 1680x945 rectangle.</li>
        <li>As it did for the old implementation, Chrome 15 seems to outshine Firefox again.</li>
        <li>Furthermore, both browsers can render arbitrary transforms (the texture mapped cube) of the noise, which is something that I would have considered impossible for real-time in the old implementation.</li>
      </ul>
      <p>How do we account for the speed improvements?  I would estimate that 99% of it is due to the change from CPU to GPU shaders; programming a highly parallelizable task on the GPU will naturally outperform its corresponding CPU-only implementation.  Moreover, the computation of Perlin noise is relatively simple, which makes it a good fit for approximation on the GPU.  I hypothesize that less JavaScript being employed might account for some of the speed improvement.</p>
      <h4>Resources</h4>
      <ul>
        <li>
